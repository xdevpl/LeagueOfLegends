...
import pl.mcdev.leagueoflegends.util.ParticleUtils.PackageType;
	...
	public static final class ParticlePacket {
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				...
				packetConstructor = ParticleUtils.getConstructor(packetClass);
				getHandle = ParticleUtils.getMethod("CraftPlayer", PackageType.CRAFTBUKKIT_ENTITY, "getHandle");
				playerConnection = ParticleUtils.getField("EntityPlayer", PackageType.MINECRAFT_SERVER, false, "playerConnection");
				sendPacket = ParticleUtils.getMethod(playerConnection.getType(), "sendPacket", PackageType.MINECRAFT_SERVER.getClass("Packet"));
		...
		private void initializePacket(Location center) throws PacketInstantiationException {
			...
			try {
				...
				if (version < 8) {
					...
					ParticleUtils.setValue(packet, true, "a", name);
				} else {
					ParticleUtils.setValue(packet, true, "a", enumParticle.getEnumConstants()[effect.getId()]);
					ParticleUtils.setValue(packet, true, "j", longDistance);
					if (data != null) {
						int[] packetData = data.getPacketData();
						ParticleUtils.setValue(packet, true, "k", effect == ParticleEffect.ITEM_CRACK ? packetData : new int[] { packetData[0] | (packetData[1] << 12) });
					}
				}
				ParticleUtils.setValue(packet, true, "b", (float) center.getX());
				ParticleUtils.setValue(packet, true, "c", (float) center.getY());
				ParticleUtils.setValue(packet, true, "d", (float) center.getZ());
				ParticleUtils.setValue(packet, true, "e", offsetX);
				ParticleUtils.setValue(packet, true, "f", offsetY);
				ParticleUtils.setValue(packet, true, "g", offsetZ);
				ParticleUtils.setValue(packet, true, "h", speed);
				ParticleUtils.setValue(packet, true, "i", amount);
			} catch (Exception exception) {
